 ...
 */
function magazinemax_customize_register($wp_customize)
{
    ...
    $widgets_section->title = esc_html__('Widgets','magazinemax');
    ...
    if (isset($wp_customize->selective_refresh)) {
        $wp_customize->selective_refresh->add_partial(
            ...
            array(
                ...
                'render_callback' => 'magazinemax_customize_partial_blogname',
            )
        );
        $wp_customize->selective_refresh->add_partial(
            'blogdescription',
            array(
                ...
                'render_callback' => 'magazinemax_customize_partial_blogdescription',
            )
        );
    }

    /*Get default values to set while building customizer elements*/
    $default_options = magazinemax_get_default_customizer_values();
    /* Header Background Color*/
    $wp_customize->add_setting(
        'magazinemax_options[header_bg_color]',
        array(
    ...
    $wp_customize->add_control(
        new WP_Customize_Color_Control(
            ...
            'magazinemax_options[header_bg_color]',
            array(
                'label' => __('Header Background Color', 'magazinemax'),
                'section' => 'colors',
                'type' => 'color',
                'priority' => 1,
    ...
    require_once get_template_directory() . '/inc/customizer/theme-options/front-page-jumbotron.php';
    require_once get_template_directory() . '/inc/customizer/theme-options/read-more.php';
    require_once get_template_directory() . '/inc/customizer/theme-options/three-col-banner-section.php';
    ...
    $wp_customize->add_section('pro__section', array(
        'title' => '' . esc_html__('View PRO Version', 'magazinemax'),
        'priority' => 2,
        'description' => sprintf(
        /* translators: %s: The view pro link. */
            __('<div class="upsell-container">
                    ...
                    </ul> %s </div>', 'magazinemax'),
            /* translators: %1$s: The view pro URL, %2$s: The view pro link text. */
            sprintf('<a href="%1$s" target="_blank" class="button button-primary">%2$s</a>', esc_url(magazinemax_get_pro_link()), esc_html__('View Magazinemax PRO', 'magazinemax'))
        ),
    ));
    $wp_customize->add_setting('magazinemax_style_view_pro_desc', array(
        'default' => '',
        'sanitize_callback' => '__return_true',
    ));
    $wp_customize->add_control('magazinemax_style_view_pro_desc', array(
        'section' => 'pro__section',
        'type' => 'hidden',
    ));
}

add_action('customize_register', 'magazinemax_customize_register');
 ...
 * @return void
 */
function magazinemax_customize_partial_blogname()
{
    bloginfo('name');
}
 ...
 * @return void
 */
function magazinemax_customize_partial_blogdescription()
{
 ...
 */
function magazinemax_customize_preview_js()
{
    wp_enqueue_script('magazinemax-customizer', get_template_directory_uri() . '/assets/js/customizer.js', array('customize-preview'), NEONMAGZIN_S_VERSION, true);

}

add_action('customize_preview_init', 'magazinemax_customize_preview_js');
 ...
 * @since 1.0.0
 */
function magazinemax_customizer_control_scripts()
{
    wp_enqueue_style('magazinemax-customizer-css', get_template_directory_uri() . '/assets/css/customizer.css');
    wp_enqueue_script('magazinemax-customizer-controls', get_template_directory_uri() . '/assets/js/customizer-admin.js', array('jquery', 'jquery-ui-sortable', 'customize-controls'));
}

add_action('customize_controls_enqueue_scripts', 'magazinemax_customizer_control_scripts', 0);

/**
 * Generate a link to the premium theme info page.
 */
function magazinemax_get_pro_link()
{
    return 'https://www.themeinwp.com/theme/magazinemax-pro/';
}