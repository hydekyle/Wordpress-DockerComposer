<?php
$ed_shop_section = magazinemax_get_option('enable_shop_section');
$section_title = magazinemax_get_option('shop_post_title');
$section_content = magazinemax_get_option('shop_post_description');
$product_type = magazinemax_get_option('shop_select_product_from');
$custom_product = magazinemax_get_option('select_product_category');
$button_lbl = magazinemax_get_option('shop_post_button_text');
$button_link = magazinemax_get_option('shop_post_button_url');
$shop_number_of_column = magazinemax_get_option('shop_number_of_column');
$shop_number_of_product = magazinemax_get_option('shop_number_of_product');
if ($ed_shop_section) {
                                ...
                                <div class="theme-product-image">
                                    ...
                                    <a href="<?php the_permalink(); ?>" rel="bookmark">
                                        ...
                                        if (has_post_thumbnail()) {
                                            ...
                                            magazinemax_theme_svg('magazinemax-shop'); //fallback
                                        }
                                        ?>
                                    </a>
                                    <?php
                                    $stock = get_post_meta(get_the_ID(), '_stock_status', true);
                                    if ($stock == 'outofstock') {
                                        echo '<span class="outofstock-label">' . esc_html__('Sold Out', 'magazinemax') . '</span>';
                                    } else {
                    ...
                    <?php if ($button_lbl && $button_link) { ?>
                        <div class="button-wrap">
                            <a href="<?php echo esc_url($button_link); ?>"
                               class="btn-readmore"><?php echo esc_html($button_lbl); ?></a>
                        </div>
                    <?php } ?>
                </div>
            <?php } ?>
